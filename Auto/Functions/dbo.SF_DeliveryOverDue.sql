...
SET QUOTED_IDENTIFIER ON
GO
 Create Function SF_DeliveryOverDue() Returns @Tmp_MainResult Table (CoId BigInt Not Null,CAmdId BigInt Not Null,PtyId BigInt Not Null,PONo Varchar(100),  PartyName Varchar(100) Not Null, SrNo Float,ItemID BigInt Not Null,IName Varchar(200) Not Null,SchQty Float ,SchDate DateTime)  As Begin Declare @AlertDate SmallDateTime Set @AlertDate = Convert(VARCHAR,GetDate(),110)  Insert Into @Tmp_MainResult (CoId,CAmdId,PtyId,PONo,PartyName,SrNo,ItemID,IName,SchQty ,SchDate)  Select CoId,CAmdId,PtyId,PONo,PartyName,SrNo,PO.ItemID,PO.IName + ' #' + I.IDrnum  ,SchQty - InvQty As SchBal,SchDate  From SF_PendingExportOrders() PO Inner Join Mst_Item I On I.ITemID = PO.ITemId Where SchQty > InvQty And SchDate <= @AlertDate Return End 
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
